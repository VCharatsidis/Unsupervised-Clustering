...
number_transformations = 16
adj_matrix = torch.ones(2 * number_transformations, 2 * number_transformations)
adj_matrix[0:number_transformations, 0:number_transformations] = 0
adj_matrix[number_transformations:, number_transformations:] = 0
...
def transformation(id, image):
    ...
    if id == 0:
        ...
        sobeled = sobel_total(color_jitter(image), quarter, True)
        ...
        sobeled = sobel_filter_x(color_jitter(image), quarter, True)
        ...
        sobeled = sobel_filter_y(color_jitter(image), quarter, True)
        ...
        t = random_crop(color_jitter(image), 22, quarter, 22)
        scaled_up = scale_up(t, 32, 32, quarter)
        sobeled = sobel_total(scaled_up, quarter, True)
        ...
        t = random_crop(color_jitter(image), 22, quarter, 22)
        scaled_up = scale_up(t, 32, 32, quarter)
        sobeled = sobel_filter_x(scaled_up, quarter, True)
...
def forward_block(X, ids, encoder, optimizer, train, rev_product):
    ...
    image_1 = transformation(aug_ids[0], image[0])
    image_2 = transformation(aug_ids[1], image[0])
    image_3 = transformation(aug_ids[2], image[0])
    image_4 = transformation(aug_ids[3], image[0])

    image_5 = transformation(aug_ids[4], image[0])
    image_6 = transformation(aug_ids[5], image[0])
    image_7 = transformation(aug_ids[6], image[0])
    image_8 = transformation(aug_ids[7], image[0])

    image_9 = transformation(aug_ids[8], image[0])
    image_10 = transformation(aug_ids[9], image[0])
    image_11 = transformation(aug_ids[10], image[0])
    image_12 = transformation(aug_ids[11], image[0])

    image_13 = transformation(aug_ids[12], image[0])
    image_14 = transformation(aug_ids[13], image[0])
    image_15 = transformation(aug_ids[14], image[0])
    image_16 = transformation(aug_ids[15], image[0])



    image_1_b = transformation(aug_ids[0], image[1])
    image_2_b = transformation(aug_ids[1], image[1])
    image_3_b = transformation(aug_ids[2], image[1])
    image_4_b = transformation(aug_ids[3], image[1])

    image_5_b = transformation(aug_ids[4], image[1])
    image_6_b = transformation(aug_ids[5], image[1])
    image_7_b = transformation(aug_ids[6], image[1])
    image_8_b = transformation(aug_ids[7], image[1])

    image_9_b = transformation(aug_ids[8], image[1])
    image_10_b = transformation(aug_ids[9], image[1])
    image_11_b = transformation(aug_ids[10], image[1])
    image_12_b = transformation(aug_ids[11], image[1])

    image_13_b = transformation(aug_ids[12], image[1])
    image_14_b = transformation(aug_ids[13], image[1])
    image_15_b = transformation(aug_ids[14], image[1])
    image_16_b = transformation(aug_ids[15], image[1])
    ...
    image_1 = torch.cat([image_1, image_2, image_3, image_4, image_5, image_6, image_7, image_8, image_9, image_10, image_11, image_12, image_13, image_14, image_15, image_16], dim=0)
    image_2 = torch.cat([image_1_b, image_2_b, image_3_b, image_4_b, image_5_b, image_6_b, image_7_b, image_8_b, image_9_b, image_10_b, image_11_b, image_12_b, image_13_b, image_14_b, image_15_b, image_16_b], dim=0)

    _, _, probs10_a = encoder(image_1.to('cuda'))
    _, _, probs10_b = encoder(image_2.to('cuda'))

    all_predictions = torch.cat([probs10_a, probs10_b], dim=0)

    current_reverse = 1 - all_predictions
    denominator = torch.cat([probs10_a.sum(dim=1), probs10_b.sum(dim=1)], dim=0)
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        for j in range(runs):
            ...
            if iteration >= 120:
                rev_product = rev_product[16 * BATCH_SIZE_DEFAULT:, :]

            if total_iters % EVAL_FREQ_DEFAULT == 0:
                print("==================================================================================")
                print("batch mean ones: ",
                      (np.where(probs10.data.cpu().numpy() > 0.5))[0].shape[0] / (number_transformations * EMBEDINGS))
                ...
                ids_to_test = np.random.choice(len(X_test), size=250, replace=False)
                test_loss = measure_acc_augments(X_test[ids_to_test, :], encoder, rev_product)
