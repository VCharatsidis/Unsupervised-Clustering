...
def new_agreement(product, denominator, rev_prod):
    ...
    diagonal_elements_bonus = 0.3 * (BATCH_SIZE_DEFAULT - 1) * (1-adj_matrix) * total_matrix

    total_matrix = total_matrix + diagonal_elements_bonus.fill_diagonal_(0)
...
def forward_block(X, ids, encoder, optimizer, train, rev_product):
    ...
    #penalty = torch.sqrt(penalty) + EPS

    penalized_binaries = all_predictions / penalty

    current_reverse = 1 - all_predictions

    denominator = torch.cat([(a/penalty).sum(dim=1), (b/penalty).sum(dim=1), (c/penalty).sum(dim=1), (d/penalty).sum(dim=1)], dim=0)
    denominator = denominator.unsqueeze(dim=1) + 20

    new_loss = new_agreement(penalized_binaries, denominator, current_reverse)
    ...
    if first or not train:
        ...
        old_loss = queue_agreement(penalized_binaries, denominator, rev_product)
...
def train():
    ...
    filepath = 'cifar100_models\\penalized_binary_contrast_4' + '.model'
