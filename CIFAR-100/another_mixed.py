...
from another_mixed_net import AnotherMixed
...
def forward_block(X, ids, encoder, optimizer, train, rev_product, moving_mean, old_product_labels, old_penalty_labels):
    ...
    # penalty_class = (c_a.sum(dim=0) + c_b.sum(dim=0) + c_c.sum(dim=0) + c_d.sum(dim=0)) / 4
    #
    # classification_loss1, current_mean = penalized_product_mean(c_a, c_b, penalty_class, moving_mean)
    # classification_loss2, current_mean = penalized_product_mean(c_a, c_c, penalty_class, moving_mean)
    # classification_loss3, current_mean = penalized_product_mean(c_a, c_d, penalty_class, moving_mean)
    #
    # classification_loss4, current_mean = penalized_product_mean(c_b, c_c, penalty_class, moving_mean)
    # classification_loss5, current_mean = penalized_product_mean(c_b, c_d, penalty_class, moving_mean)
    #
    # classification_loss6, current_mean = penalized_product_mean(c_c, c_d, penalty_class, moving_mean)
    #
    penalty_loss = penalty_loss1 + penalty_loss2 + penalty_loss3 + penalty_loss4 + penalty_loss5 + penalty_loss6
    classification_loss, penalty_class = penalized_product_generalized(c_a, c_b, c_c, c_d)


    #penalty_loss, _ = class_mean_probs_loss(pc_a, pc_b, pc_c, pc_d, moving_mean)
    #classification_loss, current_mean = class_mean_probs_loss(c_a, c_b, c_c, c_d, moving_mean)
    ...
    if train:
        moving_mean = 0.8 * moving_mean.cuda() + 0.2 * penalty_class.detach()
...
def train():
    ...
    filepath = 'cifar100_models\\another_continuous_mixed' + '.model'
    clusters_net_path = os.path.join(script_directory, filepath)

    encoder = AnotherMixed(3, EMBEDINGS, CLASSES).to('cuda')
    ...
    moving_mean = torch.ones(CLASSES) * expected
