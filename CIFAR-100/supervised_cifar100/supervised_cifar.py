...
def adjacency_matrix(ids, y_train):
    adj_matrix = torch.ones([BATCH_SIZE_DEFAULT, BATCH_SIZE_DEFAULT])
...
def swaper(ids, y_train):
    swaped_ids = copy.deepcopy(ids)
    for i in range(BATCH_SIZE_DEFAULT):
        y_i = y_train[swaped_ids[i]]
        for j in range(i+1, BATCH_SIZE_DEFAULT):
            id_j = swaped_ids[j]
            if y_i == y_train[id_j]:
                #temp = swaped_ids[id_j]
                swaped_ids[j] = swaped_ids[i]
                swaped_ids[i] = id_j
                break
...
def forward_block(X, y_train, ids, encoder, optimizer, train, rev_product):
    ...
    # swaped_ids = swaper(ids, y_train)
    # swaped_images = X[swaped_ids, :]
    ...
    adj_matrix = adjacency_matrix(ids)
    current_rev = torch.abs(adj_matrix - product)
    denominator = probs10_a.sum(dim=1) + probs10_b.sum(dim=1) + probs10_c.sum(dim=1) + probs10_d.sum(dim=1)

    total_loss = new_agreement(product, denominator, current_rev)
