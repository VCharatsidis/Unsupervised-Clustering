...
from train_predictable_reps import forward_decoder
import os
import copy
...
base_conv_model = os.path.join(script_directory, 'encoder.model')
...
guesser_model = os.path.join(script_directory, 'guesser.model')
guesser = torch.load(guesser_model)
...
def loss_representations(member_ids):
    with torch.no_grad():
        loss_list, distance, guess_loss, res, flatten_convolved_image = forward_decoder(X_test, member_ids, encoder, guesser, decoder, fake_optimizers, False)
        return loss_list, distance, res, flatten_convolved_image
...
def get_centroids(member_numbers):
    ...
    X = []
    for i in member_ids:
        loss_list, _, res, _ = loss_representations([i])
        X.append(res)
