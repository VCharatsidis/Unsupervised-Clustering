...
name = "binary_contrast_4_16_b3_lr0.00025_batch128_2linear_1.model"
...
query_images = torch.from_numpy(X_test)
query_images /= 255

galery_images = torch.from_numpy(X_Train)
galery_images /= 255
...
def get_mAP_and_Top1(bin_querry, bin_galery):
    ...
    for i in range(targets.shape[0]):
        ...
        if class_a == y_train[index_top1]:
            sum_top1 += 1

        if class_a == y_train[index_500]:
            sum_top500 += 1

        if class_a == y_train[index_1000]:
            sum_bottom1000 += 1
