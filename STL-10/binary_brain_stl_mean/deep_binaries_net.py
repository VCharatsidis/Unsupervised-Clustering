...
from deep_binaries import DeepBinBrainSTL
...
def new_binary(preds_1, preds_2):
    product = preds_1 * preds_2
    sum_product = product.sum(dim=1)

    sum_pred_1 = sum_product / EMBEDING_SIZE
...
def binary_loss(preds_1, preds_2):
    ...
    #rev_prod = 1 - product
    # negative = agreement(product, rev_prod)
    # total_loss = 0.8 * mean_energy_loss + 0.2 * negative #+ 0.03 * batch_vertical_mean

    total_loss = new_agreement(preds_1, preds_2)
...
def disagreement(product, rev_prod):
    ...
    log_nondiag = - torch.log(1-nondiag)
...
def new_agreement(preds_1, preds_2):
    product = preds_1 * preds_2
    rev_prod = 1 - product
    ...
    nondiag = nondiag / (5 * (preds_1.sum(dim=1) + preds_2.sum(dim=1)))
...
def agreement(product, rev_prod):
    transposed = rev_prod.transpose(0, 1)
    nondiag = torch.mm(product, transposed)
    poruct_sum_per_prediction = product.sum(dim=1)

    nondiag = nondiag / (EMBEDING_SIZE//60 + poruct_sum_per_prediction)
...
def forward_block(X, ids, encoder, optimizer, train):
    ...
    attraction = new_binary(test_preds_1, test_preds_2)
    # rev_attarction = new_binary(1 - test_preds_1, 1 - test_preds_2)
    #
    # repel_1 = agreement(test_preds_1, 1-test_preds_1)
    # repel_2 = agreement(test_preds_2, 1-test_preds_2)
    #
    # test_total_loss = 0.9 * rev_attarction + 0.1 * attraction + (repel_1 + repel_2) / 2
...
def train():
    ...
    encoder = DeepBinBrainSTL(1, EMBEDING_SIZE).cuda()
