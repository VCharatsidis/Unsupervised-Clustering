...
def attraction(preds_1, preds_2):
...
def forward_block(X, ids, encoder, optimizer, train, fixed_binaries=[]):
    ...
    if train:
        product = preds_1 * preds_2
        fixed_targets = fixed_binaries[ids, :].cuda()
        attraction_res = attraction(product, fixed_targets)
        rev_attarction = attraction(1 - product, 1 - fixed_targets)

        total_loss = 0.02 * attraction_res + 0.98 * rev_attarction

        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
    else:
        attraction_res = attraction(preds_1, preds_2)
        rev_attarction = attraction(1 - preds_1, 1 - preds_2)

        total_loss = 0.02 * attraction_res + 0.98 * rev_attarction

    return preds_1, preds_2, total_loss
...
def train():
    ...
    for i in range(X_train.shape[0]):
        zeros = np.zeros([EMBEDING_SIZE])
        ids = np.random.choice(EMBEDING_SIZE, size=6, replace=False)
        zeros[ids] = 1

        fixed_binaries.append(zeros)

    fixed_binaries = np.array(fixed_binaries)
    fixed_binaries = torch.from_numpy(fixed_binaries)
    ...
    for iteration in range(MAX_STEPS_DEFAULT):
        ...
        train = True
        test_preds_1, test_preds_2, loss = forward_block(X_train, ids, encoder, optimizer, train, fixed_binaries)
